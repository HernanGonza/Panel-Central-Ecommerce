...
export interface Promotion {
  ...
  active: boolean;
  /** Si está seteada, la promo aplica solo a esta categoría (ignorado si hay productIds). */
  category?: string | undefined;
  /** Si está seteado, la promo aplica solo a estos productos — tiene prioridad sobre `category`. */
...
export function promotionStatus(
  promotion: Pick<Promotion, "active" | "startDate" | "endDate">,
): PromotionStatus {
...
/** Texto corto para mostrar a qué aplica la promo (toda la tienda / una categoría / N productos). */
export function promotionScopeLabel(
  promotion: Pick<Promotion, "category" | "productIds">,
  productCount?: number,
): string {
  if (promotion.productIds && promotion.productIds.length > 0) {
    const count = productCount ?? promotion.productIds.length;
    return count === 1 ? "1 producto" : `${count} productos`;
  }
  if (promotion.category) return promotion.category;
