...
export function useShippingProviders() {
  return useQuery({ queryKey: ["shipping-providers"], queryFn: () => repositories.shippingProviders.list() });
}

export function useCreateShippingProvider() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: Omit<ShippingProvider, "id">) => repositories.shippingProviders.create(input),
    onSuccess: () => {
